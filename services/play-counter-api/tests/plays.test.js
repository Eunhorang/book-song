...
test("곡 번호와 UUID 형식을 제한한다", () => {
  ...
  assert.equal(__testables.TRACK_ID.test("12"), false);
...
test("GET은 11곡 누적 재생수를 반환한다", async () => {
...
test("11번 곡의 유효한 POST 결과를 정규화한다", async () => {
  await withEnvironment(async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(
      JSON.stringify([{ track_id: "11", play_count: 11, counted: true }]),
      { status: 200, headers: { "Content-Type": "application/json" } },
    );
    try {
      const res = response();
      ...
      assert.equal(res.result.statusCode, 200);
      assert.deepEqual(res.result.body, { trackId: "11", playCount: 11, counted: true });
    } finally {
      globalThis.fetch = originalFetch;
    }
  });

...
test("곡 번호와 UUID 형식을 제한한다", () => {
  ...
  assert.equal(__testables.TRACK_ID.test("13"), false);
...
test("GET은 12곡 누적 재생수를 반환한다", async () => {
...
test("12번 곡의 유효한 POST 결과를 정규화한다", async () => {
  await withEnvironment(async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(
      JSON.stringify([{ track_id: "12", play_count: 12, counted: true }]),
      { status: 200, headers: { "Content-Type": "application/json" } },
    );
    try {
      const res = response();
      ...
      assert.equal(res.result.statusCode, 200);
      assert.deepEqual(res.result.body, { trackId: "12", playCount: 12, counted: true });
    } finally {
      globalThis.fetch = originalFetch;
    }
  });

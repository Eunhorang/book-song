...
- `GET /api/plays`: 01~13번 전체 누적값 조회
- `POST /api/plays`: `{ trackId, eventId, playedSeconds }` 검증 후 원자적 1회 증가
- `OPTIONS /api/plays`: GitHub Pages와 로컬 QA용 CORS 사전 요청

30초 미만, 01~13 이외의 번호, UUID v4가 아닌 이벤트 ID, 허용되지 않은 브라우저 Origin은 거절합니다. 중복 이벤트 ID는 Supabase 기본키와 RPC 함수가 한 번만 반영합니다.
